...
class Carga{
	...
	public function gastos()
	{
		...
		$sleepTime = 2;  // Tiempo de espera entre reintentos en segundos

		// Eliminar registros anteriores fuera de la transacción
		Gasto::model()->deleteAllByAttributes(['chipax' => 0]);

		try {
			$this->processExpenses(GastoCompleta::POLICY_COMBUSTIBLES, $errores, $maxRetries, $sleepTime);
			$this->processExpenses(GastoCompleta::POLICY_MAQUINARIA, $errores, $maxRetries, $sleepTime);

			// Eliminar los extras una vez completada la importación de gastos
	...
	private function processExpenses($policy, &$errores, $maxRetries, $sleepTime)
	{
		$limite = 1;
		$primero = true;
		for ($i = 1; $i <= $limite; $i++) {
			$resultado = Tools::getExpenses($i, $policy);
			if ($primero) {
			...
			foreach ($expenses as $expense) {
				// Chequear política de gastos
				if ((int)$expense->ExpensePolicyId != $policy) {
					continue;
				}
				$attempt = 0;
				while ($attempt < $maxRetries) {
					...
					$transaction = $connection->beginTransaction();
					try {
						$this->processSingleExpense($expense, $errores);
						$transaction->commit();
						break; // Salir del bucle de reintentos si tiene éxito
					} catch (CDbException $e) {
						if ($e->getCode() == 1205) { // Código de error para Lock wait timeout
							$attempt++;
							sleep($sleepTime);
						} else {
							$transaction->rollback();
							throw $e; // Rethrow exception si no es un problema de bloqueo
						}
					} catch (Exception $e) {
						$transaction->rollback();
						throw $e;
				...
				if ($attempt == $maxRetries) {
					throw new Exception('No se pudo completar la operación después de múltiples intentos debido a bloqueo de tabla.');
				}
			}
		}
	}

	private function processSingleExpense($expense, &$errores)
	{
		...
		if (!$gasto->save()) {
			...
			if (isset($expense->ExtraFields)) {
				foreach ($expense->ExtraFields as $extra) {
					...
					if (!$extra_gasto->save()) {
						...
						$this->updateGastoCompleta($gasto, $extra_gasto, $errores);
					}
				}
			}
			...
			$this->finalizeGastoCompleta($gasto, $gasto_completa, $errores);
		}
	}

	private function updateGastoCompleta($gasto, $extra_gasto, &$errores)
	{
		$gasto_completa = GastoCompleta::model()->findByAttributes(['gasto_id' => $gasto->id]);
		if (!isset($gasto_completa)) {
			$gasto_completa = new GastoCompleta();
		...
		$name = strtolower(trim($extra_gasto->name));
		switch ($name) {
			case "10% impto. retenido":
			...
			case "vehiculo o equipo":
				...
				if ($valor == "Taller (vehículo virtual para registrar tosdos los gastos excepto combustibles que son de Taller y que no pueden cargarse directamente a ningún equipo o vehic.)") {
					$valor = "Taller (virtual no comb.)";
				}
				$gasto_completa->vehiculo_equipo = $valor;
				break;
			case "vehículo oficina central":
	...
	private function finalizeGastoCompleta($gasto, $gasto_completa, &$errores)
	{
		if ($gasto_completa->centro_costo_faena == null && $gasto_completa->faena != null) {
		...
		// Calcula y guarda total_calculado
