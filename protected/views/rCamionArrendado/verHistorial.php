...
<table class="table1">
    ...
    <tbody>
        <?php 
            $validaciones = HistorialValidacionesCa::model()->findAllByAttributes(array('rCamionArrendado_id'=>$report->id));
            foreach($validaciones as $validacion):
                $modificacion = ModCaAutorizadaPor::model()->findByAttributes(array('historial_validaciones_ca_id'=>$validacion->id));
                ?>
        <tr>
            <td><?php echo date_format(date_create($validacion->rCamionArrendado->fecha),"d/m/Y");?></td>
            <td><?php echo $validacion->rCamionArrendado->reporte;?></td>
