...
<script language="javascript" type="text/javascript">
	$(function() {
		...
		$('#habilitar').click(function(e) {
			$.ajax({
				type: "POST",
				url: "<?php echo Yii::app()->createUrl('//rCamionArrendado/unlock/'); ?>",
				data: {
					admin1: $('#RCamionArrendado_administrador_1').val(),
					admin2: $('#RCamionArrendado_administrador_2').val(),
					aut1: $('#RCamionArrendado_clave_admin_1').val(),
					aut2: $('#RCamionArrendado_clave_admin_2').val(),
				...
				if (msg != 'OK') {
					...
					window.location = "<?php echo CController::createUrl('//rCamionArrendado/update/' . $model->id); ?>";
												...
												<td width="100px">
													<table style="border:solid 1px silver;padding:10px;">
														<tr>
															...
															<td><?php echo $form->dropDownList($expedicion, "[$i]faena_id", CHtml::listData(Faena::model()->listar($expedicion->faena_id), 'id', 'nombre'), array('id' => "faena_id$i", 'disabled' => $model->validado == 1 || $model->validado == 2 ? 'disabled' : '', 'class' => 'faena')); ?></td>
															<td>
															...
															<td><select name="ViajeCamionArrendado[<?php echo $i; ?>][origendestino_faena_id]" class="origenDestino" <?php echo $model->validado == 1 || $model->validado == 2 ? 'disabled' : ''; ?> id="origenDestino<?php echo $i; ?>"></select></td>
												...
												<tr class="templateContent">
													<td width="100px">
														...
														<table style="border:solid 1px silver;padding:10px;">
															<tr>
															  <td><?php echo $form->labelEx($expedicion, "faena_id", array('style' => 'width:80px;')); ?></td>
															  <td><?php echo $form->dropDownList($expedicion, '[{0}]faena_id', CHtml::listData(Faena::model()->listar(), 'id', 'nombre'), array('id' => 'faena_id{0}', 'class' => 'faena', 'disabled' => $model->validado == 1 || $model->validado == 2 ? 'disabled' : '',)); ?></td>
														...
														<tr>
															...
															<td>
																...
																<?php if ($model->validado == 0) : ?>
																	<div class="remove" tipo="expedicion" id="removeExpedicion<?php echo $i; ?>" validate="true">Eliminar</div>
																<?php else : ?>
																	<div tipo="expedicionT" id="removeExpedicion<?php echo $i; ?>" validate="true"></div>
																<?php endif; ?>
															</td>
														</tr>
														<tr>
