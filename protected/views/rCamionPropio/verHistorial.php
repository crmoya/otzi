...
<table class="table1">
    ...
    <tbody>
        <?php 
            $validaciones = HistorialValidacionesCp::model()->findAllByAttributes(array('rCamionPropio_id'=>$report->id));
            foreach($validaciones as $validacion):
                $modificacion = ModCpAutorizadaPor::model()->findByAttributes(array('historial_validaciones_cp_id'=>$validacion->id));
                ?>
        <tr>
            <td><?php echo date_format(date_create($validacion->rCamionPropio->fecha),"d/m/Y");?></td>
            <td><?php echo $validacion->rCamionPropio->reporte;?></td>
