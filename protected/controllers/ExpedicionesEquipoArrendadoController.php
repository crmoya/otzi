...
class ExpedicionesEquipoArrendadoController extends Controller {
	...
	public function actionAdmin() {
		...
		$reports = ExpedicionesEquipoArrendado::model()->findAll($model->search());
		...
		foreach ($reports as $report) {
			...
			foreach ($expediciones as $expedicion) {
				...
				$produccion = ($expedicion->requipoarrendado->hFinal - $expedicion->requipoarrendado->hInicial) * $expedicion->unidadfaenaEquipo->pu;
			...
			//repuesto
			$compras = CompraRepuestoEquipoArrendado::model()->findAllByAttributes(['rEquipoArrendado_id' => $report['id']]);
			foreach ($compras as $compra) {
				$repuestos += $compra->montoNeto;
			}

			//remuneraciones
			$sueldos = RemuneracionEquipoArrendado::model()->findAllByAttributes(['rEquipoArrendado_id' => $report['id']]);
		...
		$remuneraciones = RemuneracionesSam::model()->with('equipoArrendado')->findAll($criteria);
